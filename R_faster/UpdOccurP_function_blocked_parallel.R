UpdOccurP_blocked_parallel <- function(
    ...
    detected,
    logZ_prior_mat,
    use_parallel = FALSE, ncores = max(1L, parallel::detectCores()-1L)
){
  ...
  stopifnot(is.matrix(detected) && all(dim(detected) == c(num_obs, num_studies)))
  detected_mat <- detected != 0L
  
  # per-call (shared, read-only)
  ...
  # --- per-study body (exactly your v2 logic, but as a function) ---
  one_study <- function(st, p_curr, occ_curr, accepted) {
    # Optional: avoid BLAS oversubscription inside workers
    if (requireNamespace("RhpcBLASctl", quietly=TRUE)) RhpcBLASctl::blas_set_num_threads(1L)
    
    # ENFORCE detected first (keeps proposals NA-safe)
    wh <- which(detected_mat[, st])
    if (length(wh)) { p_curr[wh, st] <- 1; occ_curr[wh, st] <- 1L }
    ...
    if (length(active_idx)) {
      p_prop_st <- p_curr_st; occur_prop_st <- occur_curr_st
      p_prop_st[active_idx] <- rtruncnorm(length(active_idx),
                                          mean = p_curr_st[active_idx],
                                          sd   = mh_p_step, a=0, b=1)
      pp <- ifelse(occur_curr_st[active_idx]==1L, 1 - p_1to0, p_0to1)
      occur_prop_st[active_idx] <- rbinom(length(active_idx), 1L, pp)
    } else {
      p_prop_st <- p_curr_st; occur_prop_st <- occur_curr_st
    }
    ...
    ## loglik/prior/proposal (active only)
    APa <- rep(-Inf, length(active_idx))
    if (length(active_idx)) {
      aidx <- active_idx
      
      # likelihood pieces (unchanged)
      lp_prop <- fast_loglik_log(p_prop_st[aidx], log_prod1[aidx], occur_prop_st[aidx])
      lp_curr <- fast_loglik_log(p_curr_st[aidx],  log_prod1[aidx], occur_curr_st[aidx])
      
      # --------- FAST: use package dtruncnorm (C) instead of pnorm logZ ----------
      log_prob_pi_prop <- log(dtruncnorm(p_prop_st[aidx], a=0, b=1,
                                          mean = pi_prior_st[aidx], sd = mh_pprior_sd) )
      log_prob_pi_curr <- log(dtruncnorm(p_curr_st[aidx], a=0, b=1,
                                          mean = pi_prior_st[aidx], sd = mh_pprior_sd) )
      log_prop_pi_prop <- log(dtruncnorm(p_prop_st[aidx], a=0, b=1,
                                          mean = p_curr_st[aidx],  sd = mh_p_step) )
      log_prop_pi_curr <- log(dtruncnorm(p_curr_st[aidx], a=0, b=1,
                                          mean = p_prop_st[aidx], sd = mh_p_step) )
      # ---------------------------------------------------------------------------
      
      # Bernoulli proposal kernels (as in v2)
      log_a  <- log(1 - p_1to0); log_1ma <- log(p_1to0)
      log_b  <- log(p_0to1);     log_1mb <- log(1 - p_0to1)
      curr1 <- occur_curr_st[aidx] == 1L
      prop1 <- occur_prop_st[aidx] == 1L
      
      log_prop_occ_prop <- numeric(length(aidx))
      log_prop_occ_prop[ curr1] <- occur_prop_st[aidx][ curr1]*log_a  + (1-occur_prop_st[aidx][ curr1])*log_1ma
      log_prop_occ_prop[!curr1] <- occur_prop_st[aidx][!curr1]*log_b  + (1-occur_prop_st[aidx][!curr1])*log_1mb
      
      log_prop_occ_curr <- numeric(length(aidx))
      log_prop_occ_curr[ prop1] <- occur_curr_st[aidx][ prop1]*log_a  + (1-occur_curr_st[aidx][ prop1])*log_1ma
      log_prop_occ_curr[!prop1] <- occur_curr_st[aidx][!prop1]*log_b  + (1-occur_curr_st[aidx][!prop1])*log_1mb
    ...
    ## accept/reject
    u_full <- runif(num_obs)
    update <- rep(FALSE, num_obs)
    if (length(active_idx)) update[active_idx] <- (log(u_full[active_idx]) < APa)
    ...
    list(p_col = p_curr[, st], occ_col = occ_curr[, st], acc_col = accepted[, st])
  }
  
  # --- parallel or serial over studies ---
  idx_all <- seq_len(num_studies)
  if (use_parallel && .Platform$OS.type == "unix" && ncores > 1L) {
    RNGkind("L'Ecuyer-CMRG"); set.seed(1)
    chunks <- split(idx_all, cut(idx_all, breaks = ncores, labels = FALSE))
    
    res <- parallel::mclapply(
      chunks, mc.cores = ncores, mc.preschedule = TRUE, mc.set.seed = TRUE,
      function(ids) {
        if (requireNamespace("RhpcBLASctl", quietly=TRUE)) RhpcBLASctl::blas_set_num_threads(1L)
        lapply(ids, function(st) {
          tryCatch({
            r <- one_study(st, p_curr, occ_curr, accepted)   # your per-study code
            # return with index for safe stitching
            list(ok=TRUE, st=st, p_col=r$p_col, occ_col=r$occ_col, acc_col=r$acc_col)
          }, error=function(e) list(ok=FALSE, st=st, msg=conditionMessage(e)))
        })
      }
    )
    
    flat <- unlist(res, recursive=FALSE)
    bad  <- vapply(flat, function(x) is.list(x) && isFALSE(x$ok), logical(1))
    if (any(bad)) {
      first <- flat[[which(bad)[1]]]
      stop(sprintf("Parallel worker error at study %d: %s", first$st, first$msg))
    }
    
    # stitch by reported index (don’t assume order)
    for (it in flat) {
      st <- it$st
      p_curr[, st]   <- it$p_col
      occ_curr[, st] <- it$occ_col
      accepted[, st] <- it$acc_col
    }
  } else { # non-parallel version
    for (st in idx_all) {
      r <- one_study(st, p_curr, occ_curr, accepted)
      p_curr[, st]   <- r$p_col
      occ_curr[, st] <- r$occ_col
      accepted[, st] <- r$acc_col
    }
  }
  ...
  list(
    ...
    occ_curr = occ_curr,
    accepted = accepted,
    timings  = c(setup = t_setup, propose = t_propose, prod1 = t_prod1,
                 loglik = t_loglik, accept = t_accept, write = t_write, enforce = t_enforce)
  )
}



# UpdOccurP_blocked_timing_v2 <- function(
#     mh_p_step, mh_pprior_sd, p_1to0, p_0to1,
#     p_curr, occ_curr,
#     occur_prior_probs, probobs_curr, probobs_others, occur_others,
#     curr_inter, 
