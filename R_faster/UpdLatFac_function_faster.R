...
UpdLatFac <- function(latfac, latfac_others,
  ...
  for (hh in 1 : Hval) {
    ...
    if (!cut_feed) { # faster version
      # add coefs_inter[h+1]^2 * rowSums( omega_inter * V[,h]^2 ) to the diagonal
      v2 <- latfac_others[, hh]^2
      row_add <- rowSums(sweep(omega_inter, 2, v2, `*`))  # length = num_obs
      idx <- seq_len(num_obs)
      new_S[idx + (idx - 1L) * num_obs] <- new_S[idx + (idx - 1L) * num_obs] +
        coefs_inter[hh + 1]^2 * row_add
    }
    
    # if (!cut_feed) { # Old slow version 
    ...
    if(num_covs[2]>0){ # DEBUGGING: ADD IF STATEMENT TO AVOID ERROR WHEN NO BINARY COVARIATES
    ...
    if (!cut_feed) {
      ...
      if (length(coefs_excl)) {
        ...
        pred_noh <- A %*% t(latfac_others[, -hh, drop = FALSE])   # num_obs x num_obs_other
      } else {
        pred_noh <- matrix(0, nrow = num_obs, ncol = num_obs_other)
      }
      pred  <- coefs_inter[1] + pred_noh
      resid <- (curr_inter - 0.5) / omega_inter - pred
      step  <- (omega_inter * resid) %*% latfac_others[, hh, drop = FALSE]  # num_obs x 1
      new_m <- new_m + as.numeric(coefs_inter[hh + 1] * step)
    }
    
    # if (!cut_feed) { # Old slower version
