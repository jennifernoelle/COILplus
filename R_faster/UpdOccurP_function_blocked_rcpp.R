...
UpdOccurP_blocked  <- function(mh_p_step = mh_p_step, mh_pprior_sd = mh_pprior_sd, 
  ...
  log1m_pipj <- log1p(-pipj)                        # NEW: precompute log(1 - p_i p_j)
  ...
  for(st in 1:num_studies){ # Loop over studies
      ...
      occ_others_st <- matrix(occur_others[, st], nrow = num_obs, ncol = ncol(curr_inter), byrow = TRUE)
      mask <- (curr_inter * focus[,, st] * occ_others_st)      # 0/1 matrix
      prod1 <- exp(rowSums(log1m_pipj * mask))                 # no pow(), no full mat exp
      
      # occ_others_st <- outer(rep(1,num_obs), occur_others[, st] )
      # mat <- (1 - pipj) ^ (curr_inter * focus[, , st] * occ_others_st)
      # prod1 <- rowwise_prod(mat) # Rcpp: take product over other species
      
      ## Proposal values
      log_prob1_prop <- fast_loglik(p_prop_st, prod1, occur_prop_st) # p(occ | pi, ...)
      ...
      ## Current values
      log_prob1_curr <- fast_loglik(p_curr_st, prod1, occur_curr_st) # p(occ | pi, ...)
