...
#' start_values = NULL
#' sampling = NULL
MCMC.rho.tau.Fixed <- function(obs_A, focus, occur_B, occur_P, obs_X, obs_W, Cu, Cv,
  ...
  if (is.null(sampling)) {  # All parameters will be updated.
    sampling <- list(L = TRUE, lambda = TRUE, tau = FALSE, beta = TRUE,
                     ...
                     z = TRUE, theta = TRUE, pis = TRUE, pjs = TRUE, rU = FALSE, # not sampling rho for animals to debug
  ...
  if (any(c(entries_X[1 : pB[1]] == 2, entries_W[1 : pP[1]] == 2))) {
    stop('Reorder covariates')  ######### ERROR 1: HERE BECAUSE CAN'T PUT CONTINUOUS COVARIATES FIRST IF THERE AREN'T ANY
  }
  ...
  this_tau_lambda <- rep(1, use_H) #1 / rgamma(use_H, 10, 10)
  this_beta <- matrix(rnorm(sum(pB) * (use_H + 1)), sum(pB), use_H + 1)
  this_gamma <- matrix(rnorm(sum(pP) * (use_H + 1)), sum(pP), use_H + 1)
  this_tau_beta <-  matrix(1, sum(pB), use_H) #matrix(1 / rgamma(sum(pB) * use_H, 10, 10), sum(pB), use_H)
  this_tau_gamma <- matrix(1, sum(pP), use_H) #matrix(1 / rgamma(sum(pP) * use_H, 10, 10), sum(pP), use_H)
  this_sigmasq_m <- 1 / rgamma(pB[1], 10, 10)
  this_sigmasq_l <- 1 / rgamma(pP[1], 10, 10)
  this_delta <- rnorm(use_H + 1, 0, 1)
  this_tau_delta <- rep(1, use_H) #1 / rgamma(use_H, 10, 10)
  this_zeta <- rnorm(use_H + 1, 0, 1)
  this_tau_zeta <- rep(1, use_H) #1 / rgamma(use_H, 10, 10)
  ...
  this_ru <- 0.9
  #this_ru <- rbeta(1, 5, 5) # FIXING RHO ANIMALS: ru = 0.9
  this_rv <- rbeta(1, 5, 5)
  this_X <- obs_X
  this_W <- obs_W
  ...
  for (ss in 1 : (Nsims * thin + burn)) {
    ...
    # ----------- tau: Update the extra variances ------------ # ERROR 2: HERE DUE TO MATRIX/ARRAY CLASS ISSUES
    ...
    # ------- beta: Coefficients of bird physical traits --------- # ERROR HERE IF ONLY ONE COVAR
    ...
    # ------- z: Latent variables for which of H components. ## 3. ERROR HERE - NA IN PROBABILITY VECTOR 
    ...
    # DOUBLE CHECKING MY IDENTITY ABOVE
    # Stabilizing transformation that seems to work
    # t_probs <- c(0.001, 0.0001, 0.00001)
    # t_log_probs <- log(t_probs)
    # t_log_probs_m <- t_log_probs - max(t_log_probs)
    # exp(t_log_probs_m)/sum(exp(t_log_probs_m))
    # t_probs/sum(t_probs)
    # 
