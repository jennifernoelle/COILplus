# New Errors: 
  ...
  # I added some extra code to detected to allow additional occurrence info
...
MCMC.trim <- function(obs_A, focus, occur_B, occur_P, obs_X, obs_W, Cu, Cv,
  ...
  if(sum(occur_B==0) + sum(occur_B==1) == length(occur_B)){
    sampling$O_B <- FALSE
  }
  
  # Similarly for the second set of species:
  if(sum(occur_P==0) + sum(occur_P==1) == length(occur_P)){
    sampling$O_P <- FALSE
  }
  ...
  # # I didn't fix it yet because it's no longer an issue in my data
  ...
  # DEBUG: if there is subsetting, we may have additional info on occurrences for species not "detected"
  detected_B_occ <- ifelse(occur_B == 1, 1, 0)
  detected_P_occ <- ifelse(occur_P == 1, 1, 0)
  
  if(sum(detected_B > detected_B_occ)){warning("Error in occur_B: All detected species should have occurrence probability = 1.")}
  if(sum(detected_P > detected_P_occ)){warning("Error in occur_P: All detected species should have occurrence probability = 1.")}
  ...
  rv_accepted <- 0
  OB_cumsum <- array(0, dim = c(nB, nstudies)) # Occurrence indicators
  OP_cumsum <- array(0, dim = c(nP, nstudies))
  
  # Extra stored quantities (see trim option)
  if(trim == FALSE){
    OP_samples <- array(NA, dim = c(Nsims, nP, nstudies))
  }
  ...
  this_O_B <- matrix(rbinom(nB * nstudies, 1, prob = occur_B), nB, nstudies) # Randomly initialize
  this_O_P <- matrix(rbinom(nP * nstudies, 1, prob = occur_P), nP, nstudies) # Randomly initialize
  ...
  for (ss in 1 : (Nsims * thin + burn)) {
    ...
    if ((ss - burn) %% thin == 0 & ss > burn) {
      ...
      if(trim == FALSE){OP_samples[keep_index, , ] <- this_O_P}
