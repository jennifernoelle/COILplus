...
UpdOccur <- function(detected, occur, probobs_curr, probobs_others, curr_inter,
  ...
  for (st in 1 : num_studies) {
    prob1 <- apply((1 - pipj) ^ (curr_inter * focus[, , st]), 1, prod) ###### ERROR st was previously ss
  ...
  # indicator to 1.
  # DEBUG: this doesn't work if we have subsetting (i.e. species may be known to be present but its partners were subset out)
