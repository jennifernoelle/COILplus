...
# TO DO HERE
  # Add traceplot code once those models finish running
  # x Add density plots for each sampler, scenario to see why recall at 50/75 differs from recall using rbinom - make pretty for supplement
    # Now for each scenario, put all of the curves on one plot with different color for each sampler, and opacity for the average density
  # x Add new file list for heatmaps and add appropriate plotting code
  # x Add new file list for occurrence indicators and add appropriate plotting code

# ---------- TO DO: set  your directories and list of models to analyze ---------#

# Paths that are the same for all models 
data_path <- 'ProcessedDataNew/'
save_plots_path <- 'ResultsNew/Plots/'
source_path <- 'HelperScriptsJKNew/'

# Set models to loop over for cross validation
model_names_cv <- c('p0_A4_old_400sims',  # 'p0_A4_old_400sims_savemore' for results with occurrence probs
                'p75_A4_old_400sims',
                'p75_B4_nb_400sims',
...
# Issue: recall is very sensitive to seed because they're so close
set.seed(10000)
set.seed(1234)
set.seed(1)
...
occ_p_plot_ridges_list <- occ_p_plot_box_list <- occ_v_plot_ridges_list <- occ_v_plot_box_list <-list()
...
for(ii in seq_along(model_names_cv)){
  ...
  m <- model_names_cv[ii]
  m_full <- model_names_full[ii] # these will usually be the same, but some issues with typos in the last run...
  ...
  results_path_cv <- paste0('ResultsNew/', m , '/')
  results_path_full <- paste0('ResultsNew/', m_full , '/')
  
  # Set sampleP and occurrence prior probs according to the version of the model fit
  sampleP <- 1- grepl('A4', m) # A4 indicates the old sampler and so occurrence probs are not sampled
  
  if(grepl('p0', m)){ # Unobserved species are assumed not present
    O_P <- ifelse(O_P_expert == 1, 1, 0)
    O_V <- ifelse(O_V_expert ==1, 1, 0)
    O_labels <- c("Different study","Same study")
  }else if(grepl('p75', m)){ # Unobserved species are assumed present with p = 0.75
  ...
  scenario <- ifelse(grepl('p0', m), "0/100%", 
                     ifelse(grepl('p75', m), "75/100%", "Expert"))
  
  # Set which sampler was used
  sampler <- ifelse(grepl("_old", m), "Old", ifelse(grepl("_b_", m), "New, blocked", "New, unblocked"))
  ...
  for (ii in 1 : nchains) {
    cat("\n Chain = ", ii)
    res <- loadRData(paste0(results_path_full, 'res_',  m_full, '_', ii, '.dat')) 
  ...
  ## Plant occurrences
  # Boxplot of post mean occurrence indicators by prior value
 pbox <- data.frame(obs_op = c(ifelse(O_P ==1, 1, 0.75)), post_op = c(mean_occ_p)) %>%
    ggplot(aes(x = as.factor(O_P), y = post_op)) + 
    geom_boxplot() + 
    xlab('Prior occ prob') + 
    ylab('Posterior mean occ prob') + 
    ggtitle(paste0("Posteior mean of plant occurrence: ", scenario, ", ", sampler)) + 
    theme_bw() + 
    theme(text = element_text(family = "serif", size = 20)) 
    
  occ_p_plot_box_list[[counter]] <- pbox
  ...
  p_ridges <- rbind(df1a, df1b)%>%
    transform(type = factor(type, levels = c("Occurrence baseline probabilities", "Occurrence scenario"))) %>% 
    ggplot(aes(x =post_op, y= color, fill = color)) + 
    geom_density_ridges(alpha = 0.75) +
    ylab("") + 
    xlab(paste0("Posterior mean of plant occurrence: ", scenario, ", ", sampler)) + 
    guides(fill = "none")+ 
    facet_wrap(~type) + 
    theme_bw() + 
    #scale_x_continuous(limits = c(0.65,0.8)) + 
    theme(text = element_text(family = "serif", size = 20))
  occ_p_plot_ridges_list[[counter]] <- p_ridges
  ...
  # Boxplot of post mean occurrence indicators by prior value
  pbox <- data.frame(obs_op = c(ifelse(O_V ==1, 1, 0.75)), post_op = c(mean_occ_v)) %>%
    ggplot(aes(x = as.factor(O_V), y = post_op)) + 
    geom_boxplot() + 
    xlab('Prior occ prob') + 
    ylab('Posterior mean occ prob') + 
    ggtitle(paste0("Posterior mean of vert occurrence: ", scenario, ", ", sampler)) + 
    theme_bw() + 
    theme(text = element_text(family = "serif", size = 20)) 
  
  occ_v_plot_box_list[[counter]] <- pbox
  ...
  p_ridges <- rbind(df1a, df1b)%>%
    ...
    geom_density_ridges(alpha = 0.75) +
    ylab("") + 
    xlab(paste0("Posteior mean of vert occurrence: ", scenario, ", ", sampler)) + 
    guides(fill = "none")+ 
    facet_wrap(~type) + 
    theme_bw() + 
    #scale_x_continuous(limits = c(0.65,0.8)) + 
    theme(text = element_text(family = "serif", size = 20))
  occ_v_plot_ridges_list[[counter]] <- p_ridges
  ...
  # For now just entirely skipping 'p75_C4_b_400sims' full results because no occurrence indicators saved
  ...
  v_group <- v.taxa.ordered$Animal_Family
  ...
  png(paste0(save_plots_path, "heatmap_", m_full, ".png"), width = 1500, height =1000)
  ...
  for (rr in 1 : repetitions) {
    cv_indices <- loadRData(paste0(results_path_cv, 'cv_indices_', m, '_', rr, '.dat'))
    pred <- loadRData(paste0(results_path_cv, 'pred_', m , '_', rr, '.dat'))
...
# OOS Performance Metrics table
recall_summary <- recall_df %>% group_by(stat, scenario, sampler) %>% 
              summarize(mean_value = mean(value))
precision_summary <- precision_df%>% group_by(stat, scenario, sampler) %>% 
  summarize(mean_value = mean(value))
