...
# Set parameters as defined during resampling
#B <- 100 # permutations
...
gridExtra::grid.arrange(g, left = textGrob("Frugivore information: Increasing Log Body Mass", rot = 90,
                                           x = 1.3, y = 0.57, gp = gpar(fontsize = 12)),
                        bottom = textGrob("Plant information: Increasing Wood Density",
...
v.families.ordered <- unique(v.taxa.ordered$Animal_Family)
v.family.codebook <- data.frame(v.family = v.families.ordered) %>% 
                     mutate(v.show.order = row_number())
...
keep_v_groups <- sort(c(names(which(v_size_cluster >= min_v_size)), "Elephantidae", "Hominidae"))
...
# Issue: superheat is going to alphabetize, but I want the rows sorted by class
plot_pred_W <- corr_obs_W[v.taxa.order,] # first sort by class and family
plot_pred_W_subset  <- plot_pred_W[keep_v_index, ] # now subset to 
v_membership <- v_group[keep_v_index]
