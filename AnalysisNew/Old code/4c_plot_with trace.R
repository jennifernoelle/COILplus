...
# 
# 
# # Other trace plots: random probabilities. Note a different one will be selected for each chain so it doesn't make sense to keep together across chains
# if(nchains > 1){
#   trace_pis <- cbind(t((all_res[[1]]$random_pis)$save_pis), t((all_res[[2]]$random_pis)$save_pis))
#   pi_indices <- rbind((all_res[[1]]$random_pis)$save_pi_indices, (all_res[[2]]$random_pis)$save_pi_indices)
#   if(nchains>2){for (cc in 3 : nchains) {
#     trace_pis <- cbind(trace_pis, t(all_res[[cc]]$random_pis$save_pis))
#     pi_indices <- rbind(pi_indices, all_res[[cc]]$random_pis$save_pi_indices)
#   }
#   }
# }else{
#   trace_pis <- (all_res[[1]]$random_pis)$save_pis
