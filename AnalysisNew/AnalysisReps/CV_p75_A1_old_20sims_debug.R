...
Nsims <- 10 #20 # original 10000, reasonable 2500
thin <-  1 #10 #original 40
burn <-  1 #floor(Nsims*thin*5) # original 40000, reasonable 2500
...
# ------------- STEP 2: Parallel setup --------------- #
  ...
  # Getting the study indices for zero-ed entries
  ...
  # DEBUG: or should we only set Focus to zero for the zero'd out true observations, i.e., not for the observed non-edges?
  # We want to essentially drop observations only where the interaction was observed?
  # Confused about why this_FO is not always 0 for cv entries
  ...
  # Debugging: make sure use_A looks right
  comb_A_check <- apply(use_A, c(1, 2), sum)
  comb_A_check <- (comb_A_check > 0) * 1
  sum(comb_A_check[cv_indices])
  ...
  focus <- use_F
  obs_A <- use_A
  mcmc <- MCMC.debug(cv_indices = cv_indices, obs_A = use_A, focus = use_F, p_occur_B, p_occur_P, obs_X, obs_W, Cu, Cv,
  ...
  # Debugging
  mod_pL1s <- apply(mcmc$mod_pL1s, c(2, 3), mean)
  df.Ls <- data.frame(post.prob = c(mod_pL1s), post.L = c(pred))
  
  pred_cv <- pred[cv_indices] # Predictions are all 1
  pL1_cv <- mod_pL1s[cv_indices] # Even though probs are widely distributed
  mean(pL1_cv)
  hist(pL1_cv)
