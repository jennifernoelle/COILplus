...
Nsims <- 5 #10000 #500 #1000 #5000 # original 10000, reasonable 2500
thin <-  1 #10 #5 # original 40
burn <-  1 #floor(Nsims*thin*5) #5 #40000 #2000 #5000 # 2000 # 22000 # original 40000, reasonable 2500
...
plot(x = c(O_P), y = c(post.pi) , main = "Posterior vs Prior",
...
set.seed(cc)
  ...
  # Maybe I should be saving the occurrence indicators too?
  # Occurrence probabilities for plants
  occ_plants <- list(OPs = mcmc$OP_mean, p_OPs = mcmc$p_OP_mean, p_accept = mcmc$p_OP_accepted) # DEBUG this is where the error was
  occ_verts <- list(OBs = mcmc$OB_mean, p_BPs = mcmc$p_OB_mean, p_accept = mcmc$p_OB_accepted) # DEBUG 
