...
repetitions <- 2 # Number of cv replications
n.cv <- 100 # Number of heldout observations

Nsims <- 2 # original 10000, reasonable 2500
thin <-  1 #original 40
burn <-  2 #floor(Nsims*thin*5) # original 40000, reasonable 2500
...
#   pred_nb <- apply(mcmc$Ls, c(2, 3), mean) # Why not save the interaction probs instead of the predictions?
...
Sys.time() - t1
# # Debugging the function without mclapply
# for(i in 1:repetitions){
#   mcmc.cv.parallel(rr=i, n.cv = n.cv, 
#                    obs_A, focus = obs_F, p_occur_B = O_V, p_occur_P = O_P, 
#                    obs_X, obs_W, Cu, Cv,
#                    Nsims, burn, thin, use_H = 10, use_shrinkage = TRUE,
#                    bias_cor = TRUE, theta_inf = 0.01,
