# TO DO: 
  # Add an option to sample occurrence probabilities or not
    # Add a sample occurrence probs slot to the sampling arg
    # Test it
  # Add an option to used blocked vs sequential samplers
  # Test it
...
#' @param p_occur_B Matrix with rows corresponding to the first set of species
...
MCMC.debug <- function(cv_indices, obs_A, focus, p_occur_B, p_occur_P, obs_X, obs_W, Cu, Cv,
  ...
  if(block_sampleOccP==TRUE & (sampling$p_OB == FALSE & sampling$p_OP == FALSE)){
    stop("Error: one or both of sampling$p_OB and sampling$p_OP must be TRUE in
  ...
  if(sum(p_occur_B==0) + sum(p_occur_B==1) == length(p_occur_B)){
    sampling$O_B <- sampling$p_OB <- FALSE
  }
  ...
  # Debugging: why aren't cv_indices in A0_indices? they should be set to 0
  # A0_indices <- arrayInd(index_A0, .dim = dim(comb_A))
  cv_mat <- matrix(0, nrow = nB, ncol = nP) # 1 if cv index,0 otherwise
  cv_mat[cv_indices] <- 1
  # sum(cv_mat[index_A0])
  # sum(comb_A * cv_mat) # this should be 0 because comb_A <- 0 for heldout obs
  ...
  detected_B_occ <- ifelse(p_occur_B == 1, 1, 0)
  detected_P_occ <- ifelse(p_occur_P == 1, 1, 0)
  
  if(sum(detected_B > detected_B_occ)){warning("Error in p_occur_B: All detected species should have occurrence probability = 1.")}
  ...
  if(sum(detected_B < detected_B_occ)){
    message("Note on p_occur_B: Some species with occurrence probability = 1 do not have observed interactions (likely due to subsetting)")
    }
  ...
  rv_accepted <- 0
  p_OB_accepted <- array(0, dim = c(nB,nstudies)) # Number of MCMC iteractions for which for bird occurrence prob proposal was accepted
  p_OB_cumsum <- array(0, dim = c(nB, nstudies))
  occP_B_accepted <- array(0, dim = c(nB, nstudies))
  #p_OBs <- array(NA, dim = c(Nsims, nB, nstudies)) # Trim this after debugging
  ...
  OB_cumsum <- array(0, dim = c(nB, nstudies))
  ...
  if(sampling$p_OB == TRUE){
    this_occur_B <- matrix(rtruncnorm(n = nB*nstudies, mean = p_occur_B, sd = mh_pprior_sd, a = 0, b = 1), 
                           nrow = nB, ncol = nstudies) # Note naming convention is off, can fix later if wanted (this_occur_B)
    this_occur_B[p_occur_B == 1] <- 1
  }else {this_occur_B <- p_occur_B} # New so that we don't randomly initialize the probs when we aren't sampling them
  ...
  this_O_B <- matrix(rbinom(nB * nstudies, 1, prob = p_occur_B), nB, nstudies) # New: initialize with prior probs
  ...
  # Reduce the dimension of stored quantities
  #cum_sum_pL1s <- matrix(data = 0, nrow = nB, ncol = nP)
  ...
  for (ss in 1 : (Nsims * thin + burn)) {
    ...
    if (sampling$L | sampling$lambda | sampling$U | sampling$V) {
      ...
      # JK Question: what is bias correction here?
      # JK: Based on the work below, it looks like it refers to the effect of detection bias
      this_mod_pL1 <- expit(logit_pijL)
    
      # Debugging code
      cv_modpL1s <- this_mod_pL1[cv_indices]
      cat("\n iter = ",ss, " CV mean unadj probs" , mean(cv_modpL1s))
    ...
    if (sampling$L) {
      ...
      # Probability of true interaction L = 1, or L = 0, when A = 0.
      # JK: we deflate the interaction probabilities above based on how many observed nonedges there were when A = 0
      pL1 <- this_mod_pL1 * (1 - pipj) ^ this_FO # JK: this_FO= number studies with focus and occurrence = 1
      pL0 <- 1 - this_mod_pL1
      this_pL1 <- pL1 / (pL0 + pL1)  # Standardize.
      
      # Update L.
      # JK: in this step we impute L when A==0 using the corrected interaction probs this_pL1 (take into account detection, observed non-edges)
      this_L <- matrix(1, nB, nP)
      this_L[index_A0] <- rbinom(n = quant_A0, 1, prob = this_pL1[index_A0])
      
      # Debugging code
      cv_pL1s <- this_pL1[cv_indices]
      cat("\n CV mean adj probs: " , mean(cv_pL1s))
      
      cv_Ls <- this_L[cv_indices]
      cat("\n CV mean L: " , mean(cv_Ls))
      
      cat("\n CV indices with A0: ", sum(cv_mat[index_A0]))
    }
    ...
    if (sampling$z) {
      ...
      for (hh in 1 : use_H) {
        ...
        #probs <- exp(log_prob) / sum(exp(log_prob)) ## ORIGINAL CODE: this returns NaN 
        log_prob_diff <- log_prob - max(log_prob) # ERROR 3: my replacement code
        probs <- exp(log_prob_diff) / sum(exp(log_prob_diff)) # ERROR 3: my replacement code
    ...
    # If using unblocked sampler, we sample occurrence indiator and probs with separate functions
    ...
    if(block_sampleOccP ==  FALSE){
      ...
      #   warning("Sampler error in p_occur_B: All detected species should have occurrence probability = 1.")
      # }
      ...
      if(sampling$p_OB){ 
        ...
        upd_P_OB <- UpdOccurP(occur_indicators = this_O_B, 
                              occur_prior_probs = p_occur_B, 
        ...
        this_occur_B <- upd_P_OB$new_value_p
        p_OB_accepted <- p_OB_accepted + upd_P_OB$accepted
    ...
    ### New steps
    # TO DO
    # Add default prior probs computation
    # Think about sampling options (does it make sense to sample occurrence probs but not occurrence, etc)
    # x Add new storage for acceptance
    # x Add options for blocked vs unblocked updating
    # x source utils and new functions
    # x Add error if block updating but no update p 
    # x Update cum sums? Unchanged
    ...
    if(block_sampleOccP){
      ...
      if(sampling$O_B == TRUE & sampling$p_OB == TRUE){
        upd_OccP_B <- UpdOccurP_blocked(mh_p_step = mh_p_step, mh_pprior_sd = mh_pprior_sd, 
                                        p_1to0 = p_1to0, p_0to1 = p_0to1, 
                                        p_curr = this_occur_B, occ_curr = this_O_B,
                                        occur_prior_probs = p_occur_B, 
     ...
     # if(mean(this_O_P[obs_OP==1])!=1){
      ...
      if(mean(this_O_B[p_occur_B==1])!=1){
        warning("Sampler error in p_occur_B: All detected species should have occurrence probability = 1.")
      }
    ...
    if ((ss - burn) %% thin == 0 & ss > burn) {
      ...
      p_OB_cumsum <- p_OB_cumsum + this_occur_B # occurrence probabilities
      p_OB_mean <- p_OB_cumsum/keep_index
      p_OP_cumsum <- p_OP_cumsum + this_occur_P 
      p_OP_mean <- p_OP_cumsum/keep_index
      
      OB_cumsum <- OB_cumsum + this_O_B  # occurrence indicators   
      OB_mean <- OB_cumsum/keep_index
      OP_cumsum <- OP_cumsum + this_O_P
      OP_mean <- OP_cumsum/keep_index
        
      #p_OBs[keep_index, , ] <- this_occur_B # trim after debugging
      #p_OPs[keep_index, , ] <- this_occur_P
      
      Ls[keep_index, , ] <- this_L
      #pL1s[keep_index, , ] <- this_pL1 #TRIM confused, I think these are the bias-corrected interaction probabilities we want
  ...
  r <- list(
            ...
            omegas = omegas, zs = zs,
            thetas = thetas, 
            p_OB_mean = p_OB_mean, # running mean of occurrence probabilities
            p_OP_mean = p_OP_mean,
            OP_mean = OP_mean, # running mean of occurrence indicators
            OB_mean = OB_mean,
            #p_OBs = p_OBs, # TRIM after debug
            #p_OPs = p_OPs, # TRIM after debug
            #pis = pis, pjs = pjs, #TRIM
            ...
            p_OB_accepted = p_OB_accepted/(Nsims * thin + burn), 
