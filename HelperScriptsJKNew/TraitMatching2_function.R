...
#' first set of species. Returned by the function MCMC. All chains stacked together.
...
TraitMatching2 <- function(B = 500, mod_pL1s, Xs, Ws, obs_X, obs_W,
  ...
  corr_obs_X <- array(NA, dim = c(Nsims, sum_pB))
  rsq_obs_W <- array(NA, dim = c(Nsims, sum_pP))
  corr_obs_W <- array(NA, dim = c(Nsims, sum_pP))
