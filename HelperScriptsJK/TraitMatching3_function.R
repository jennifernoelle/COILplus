...
TraitMatching2 <- function(B = 500, mod_pL1s, Xs, Ws, obs_X, obs_W,
  ...
  for (mm in 1 : sum_pB) {
    ...
    for (ss in 1 : Nsims) {
      ...
      # This tells us if bird covariate mm is associated with higher or lower interaction proclivities 
      rsq <- rep(NA, nP)
      for (jj in 1 : nP) {
    ...
    # In progress
    # For each species of type 2: compute variable importance of species type 1 covars
    corr <- rep(NA, nP) # JK: adding this to investigate sign of correlation coefficeints
    for (jj in 1 : nP) {
      this_response <- logit_mod_pL1s[, ,jj] # interaction probs at iteration ss
      ...
      if (length(wh_na) > 0) {
        if (obs_only) {
          ...
          this_cov[wh_na] <- Xs[[mm]][ss, ] # not sure how to modify this line
        }
      }
      
      corr[jj] <- cor(this_response[, jj], this_cov) # corr (interaction probs for plant jj, covariate mm for corresonding birds)
    }
  ...
  for (ll in 1 : sum_pP) {
    ...
    for (ss in 1 : Nsims) {
      ...
      corr <- rep(NA, nB)
      for (ii in 1 : nB) {
        corr[ii] <- cor(this_response[ii, ], this_cov)
        rsq[ii] <- cor(this_response[ii, ], this_cov) ^ 2
      }
      rsq_obs_W[ss, ll] <- mean(rsq)
      corr_obs_W[ss, ll] <- mean(rsq)
