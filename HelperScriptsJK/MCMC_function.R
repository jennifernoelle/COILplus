...
MCMC <- function(obs_A, focus, occur_B, occur_P, obs_X, obs_W, Cu, Cv,
  ...
  for (ss in 1 : (Nsims * thin + burn)) {
    ...
    if (sampling$L) {
      ...
      # Update L.
      # JK: in this step we impute L when A==0 using the corrected intreaction probs this_pL1 (take into account detection, observed non-edges)
    ...
    if ((ss - burn) %% thin == 0 & ss > burn) {
      ...
      #cum_sum_pL1s <- cum_sum_pL1s + this_pL1 # previously + this_mod_pL1 (but that was without bias correction)
      #pL1s_mean <- cum_sum_pL1s/keep_index
  ...
  r <- list(
            Ls = Ls, pL1s = pL1s, mod_pL1s = mod_pL1s, 
            #pL1s_mean = pL1s_mean,
