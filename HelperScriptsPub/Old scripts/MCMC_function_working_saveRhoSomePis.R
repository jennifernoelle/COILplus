...
#' interactions: this version only saves rho_u, rho_v, and a few random pis for trace plots
#' 
...
#' @param trace_pis Optional matrix giving the indices of cells to save all posterior samples
#' for making trace plots
#' 
#' @export
#' 
#' 
...
MCMC <- function(obs_A, focus, p_occur_B, p_occur_P, obs_X, obs_W, Cu, Cv,
  ...
  rU <- rep(NA, Nsims*thin + burn)  # Correlation, latent factors for birds.
  ru_accepted <- 0
  ...
  ### Setup storage for 9 random non-1 probabilities
  
  # # Getting the indices: now these are set as an argument to the function
  ...
  # 
  # Now set up storage
  save_pis <- matrix(NA, nrow = 9, ncol = burn + thin*Nsims)
  ...
  for (ss in 1 : (Nsims * thin + burn)) {
    ...
    if (sampling$L) {
      ...
      # JK: we defate the interaction probabilities above based on how many observed nonedges there were when A = 0
    ...
    if(block_sampleOccP ==  FALSE){
      ...
      if(mean(this_O_B[p_occur_B==1])!=1){
        warning("Sampler error in O_B: All detected species should have occurrence always = 1.")
      }
      ...
      if(mean(this_occur_B[p_occur_B==1])!=1){
    ...
    # In this version we only save the rho's for traceplots and we don't bother thinning or discarding burn-in
    #if ((ss - burn) %% thin == 0 & ss > burn) {
    
    # Save a few random probabilities for trace plots  
    for(ii in 1:9){
      save_pis[ii, keep_index] <- this_mod_pL1[pi_trace_indices[ii, 1], pi_trace_indices[ii, 2]]
    }
