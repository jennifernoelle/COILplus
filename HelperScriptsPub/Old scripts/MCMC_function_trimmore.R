# New Errors: 
  ...
  # This version has the prior on occurrence probs
...
MCMC.trimmore.new <- function(obs_A, focus, p_occur_B, p_occur_P, obs_X, obs_W, Cu, Cv,
  ...
  # ERROR: this is not quite right because we can have 2 unique values like 0.5, 0.75  
  # And in that case we should sample, whereas by default they would just be fixed at
  # Initialized Bernoulli draws 
  # if (length(unique(as.numeric(occur_B))) <= 2) { # check if binary 0/1
  #   sampling$O_B <- FALSE
  #}
  ...
  # this_occur_B <- matrix(rtnorm(n = nB*nstudies, mean = p_occur_B, sd = mh_occ_sd, lower = 0, upper = 1), 
  #                   nrow = nB, ncol = nstudies) # Note naming convention is off, can fix later if wanted (this_occur_B)
  this_occur_B <- matrix(rtruncnorm(n = nB*nstudies, mean = p_occur_B, sd = mh_occ_sd, a = 0, b = 1), 
                         nrow = nB, ncol = nstudies) # Note naming convention is off, can fix later if wanted (this_occur_B)
  this_occur_B[p_occur_B == 1] <- 1
  # this_occur_P <- matrix(rtnorm(n = nP*nstudies, mean = p_occur_P, sd = mh_occ_sd, lower = 0, upper = 1), 
  #                   nrow = nP, ncol = nstudies) # Note naming convention is off, can fix later if wanted (this_occur_B)
  this_occur_P <- matrix(rtruncnorm(n = nP*nstudies, mean = p_occur_P, sd = mh_occ_sd, a = 0, b = 1), 
  ...
  for (ss in 1 : (Nsims * thin + burn)) {
    ...
    ## DEBUGGING
    if(mean(this_O_P[obs_OP==1])!=1){warning("Error in p_occur_B: All detected species should have occurrence probability = 1.")}
    ...
    # TO DO
      # x Add sampling args
      # x Add prior prob arg and use it to initialize occur_B, occur_P
      # x Add MH args
      ...
      # x Initialize accept prob
      # x Add accept prob computation and make return value
      # x Add storage for and return value, running mean for post probs (remove storage after debugging)
      # x Add return value for post probs
