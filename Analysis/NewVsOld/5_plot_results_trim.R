...
## Subset plants to simplify for testing new function
...
p_occ <- array(NA, dim = c(nchains*use_Nsims, nP, nS))
p_accept <- array(NA, dim = c(nchains, nP, nS))
...
for (ii in 1 : nchains) {
  ...
  p_occ[1 : use_Nsims + use_Nsims * (ii - 1), , ] <- all_res[[ii]]$occ_plants[[1]] 
  p_accept[ii, , ] <- all_res[[ii]]$occ_plants[[2]]
}

dimnames(pred_ours)[2 : 3] <- list(mammal = rownames(obs_A), plant = colnames(obs_A))
dimnames(p_occ)[2:3] <- list(plant = colnames(obs_A), study = s.names)
...
#-------------------- STEP 1B: SANITY CHECK: interactions -----------------------------#
...
for(p in 1:12){
  plant <- sample(1:nP,1)
  study <- sample(1:nS, 1)
  plot(p_occ[,plant, study], type = "l", main = paste0(colnames(obs_A)[plant], " x ", s.names[study]))
...
# Debug: why is Irvingia_gabonensis x study.71 being sampled when it is a 1?
obs_OP[, which(s.names == "Study.71")]
p_occ[,which(plant.names == "Irvingia_gabonensis"), which(s.names == "Study.71")]
