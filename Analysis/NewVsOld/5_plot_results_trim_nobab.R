...
if(save_files == TRUE){write.csv(mean_pred, paste0(results_path, "PosteriorNetwork_", date, ".csv"))}


# For plotting: set the recorded interactions to NA (so that they don't overpower the colors)
mean_pred_na[comb_A == 1] <- NA


#-------------------- STEP 1B: SANITY CHECKS -----------------------------#
...
png(paste0(results_path, "PosteriorNetworks/heatmap_imputed", date, ".png"), width = 1000, height =1000)
...
# Plot observed frugivory network 
png(paste0(results_path, "PosteriorNetworks/heatmap_observed", date, ".png"), width = 1000, height =1000)
...
# latent factors for mammal and plant species:
apply(all_cor, 2, mean)
apply(all_cor, 2, quantile, probs = c(0.025, 0.975))

# Mixing is not great, run longer
plot(all_cor[,1,1], type = "l") # rho_u mixing: better
plot(all_cor[,2,1], type = "l") # rho_v mixing: not great
