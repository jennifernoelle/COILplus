...
load(paste0(data_path, 'traits_p_709_clean.dat')) # "traits.plants"
...
traits.p <- traits.plants[wh_keep_p, ]
...
png(paste0(results_path, "Figures/heatmap_cv_", date, ".png"), width = 750, height = 750 )
...
sum(comb_A)
sum(comb_A)/length(comb_A) # proportion of pairs with known interactions
(sum(plot_pred > 0.75, na.rm = TRUE) + sum(comb_A))/length(plot_pred) # proportion of pairs with predicted interactions, "very likely"
sum(plot_pred > 0.75, na.rm = TRUE)
(sum(plot_pred > 0.5, na.rm = TRUE) + sum(comb_A))/length(plot_pred) # proportion of pairs with predicted interactions, "likely"
