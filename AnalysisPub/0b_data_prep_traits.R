# Clean the supplementary traits data to produce the following: Obs_X, Obs_W
...
codebook.v <- read.csv(paste0(save_path, 'v_taxa_codebook.csv')) %>% 
  select(Animal_Species_Corrected, Animal_Species_Generic)
sum(codebook.v$Animal_Species_Corrected != rownames(Obs_X))
Obs_X_generic <- Obs_X
rownames(Obs_X_generic) <- codebook.v$Animal_Species_Generic

codebook.p <- read.csv(paste0(save_path, 'p_taxa_codebook.csv')) %>% 
  select(Plant_Species_Corrected, Plant_Species_Generic)
sum(rownames(Obs_W) != codebook.p$Plant_Species_Corrected)
Obs_W_generic <- Obs_W
rownames(Obs_W_generic) <- codebook.p$Plant_Species_Generic
...
if (save_files) {
  ...
  save(Obs_X_generic, file = paste0(save_path_generic, 'Obs_X.dat'))
  save(Obs_W_generic, file = paste0(save_path_generic, 'Obs_W.dat'))
}
