# This file prepares the frugivore phylogenetic correlation matrix: Cu_phylo
...
codebook <- read.csv(paste0(save_path, 'v_taxa_codebook.csv')) %>% 
  select(Animal_Species_Corrected, Animal_Species_Generic)
sum(rownames(Cu_phylo) != codebook$Animal_Species_Corrected)
Cu_phylo_generic <- Cu_phylo
rownames(Cu_phylo_generic) <- colnames(Cu_phylo_generic) <- codebook$Animal_Species_Generic

if(save_files){
  save(Cu_phylo_generic, file = paste0(save_path_generic, 'Cu_phylo.dat'))
