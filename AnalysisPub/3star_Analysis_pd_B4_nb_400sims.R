...
# Rerunning with the old settings to reproduce traceplots for Camille
...
source(paste0(source_path, 'MCMC_function_working_saveless.R'))
...
mcmc.parallel <- function(cc, obs_A, focus = obs_F, p_occur_B = O_V, p_occur_P = O_P, obs_X, obs_W, Cu, Cv,
                          ...
                          sampling = sampling,
                          cut_feed = FALSE,
                          block_sampleOccP = block_sampleOccP, save_logL = save_logL){
...
set.seed(cc)
  ...
  # 
  # Save logL for traceplots: all we care about this time
  logL <- mcmc$logL
  
  # Combining the results we are interested in to a list and saving:
  # res <- list(all_pred = all_pred, correlations = correlations, 
  #             p_detect = p_detect, factors = factors, occ_plants = occ_plants, 
  #             occ_verts = occ_verts,  Xs = Xs, Ws = Ws, logL = logL
  #             )
  res <- list(logL = logL)
